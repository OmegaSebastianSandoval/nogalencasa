...
<div class="container-fluid">
	<form action="<?php echo $this->route . "?producto=" . $this->producto . ""; ?>" method="post">
			...
			<div class="row">
				<div class="col-3">
					...
					<label class="input-group">
						...
						<input type="text" class="form-control" name="acomp_nombre"
							value="<?php echo $this->getObjectVariable($this->filters, 'acomp_nombre') ?>"></input>
					</label>
				</div>
				<div class="col-3">
					<label>tipo</label>
					<label class="input-group">
						...
						<select class="form-control" name="acomp_tipo">
							<option value="">Todas</option>
							<?php foreach ($this->list_acomp_tipo as $key => $value): ?>
								<option value="<?= $key; ?>" <?php if ($this->getObjectVariable($this->filters, 'acomp_tipo') == $key) {
										echo "selected";
									} ?>><?= $value; ?></option>
							<?php endforeach ?>
						</select>
	...
	<div align="center">
		<ul class="pagination justify-content-center">
			...
			if ($this->totalpages > 1) {
				if ($this->page != 1)
					echo '<li class="page-item" ><a class="page-link"  href="' . $url . '?page=' . ($this->page - 1) . '&producto=' . $this->producto . '"> &laquo; Anterior </a></li>';
				for ($i = 1; $i <= $this->totalpages; $i++) {
					if ($this->page == $i)
						echo '<li class="active page-item"><a class="page-link">' . $this->page . '</a></li>';
					else
						echo '<li class="page-item"><a class="page-link" href="' . $url . '?page=' . $i . '&producto=' . $this->producto . '">' . $i . '</a></li>  ';
				}
				if ($this->page != $this->totalpages)
					echo '<li class="page-item"><a class="page-link" href="' . $url . '?page=' . ($this->page + 1) . '&producto=' . $this->producto . '">Siguiente &raquo;</a></li>';
			}
			?>
		</ul>
	</div>
			...
			<div class="row">
				...
				<div class="col-3">
					<div class="text-right"><a class="btn btn-sm btn-success"
							href="<?php echo $this->route . "\manage" . "?producto=" . $this->producto . ""; ?>"> <i
			...
			<table class=" table table-striped  table-hover table-administrator text-left">
				...
				<tbody>
					<?php foreach ($this->lists as $content) { ?>
						<?php $id = $content->acomp_id; ?>
						<tr>
							<td><?= $content->acomp_nombre; ?></td>
							<td><?= $this->list_acomp_tipo[$content->acomp_tipo]; ?></td>
							<td>
										...
										<div class="modal-content">
											...
											<div class="modal-footer">
												...
												<a class="btn btn-danger"
													href="<?php echo $this->route; ?>/delete?id=<?= $id ?>&csrf=<?= $this->csrf; ?><?php echo '' . '&producto=' . $this->producto; ?>">Eliminar</a>
											</div>
										</div>
									</div>
								</div>

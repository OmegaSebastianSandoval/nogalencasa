...
class Administracion_usuarioController extends Administracion_mainController
{
	...
	private function getUserlevel()
	{
		...
		$array['2'] = 'Comunicaciones';
		$array['3'] = 'Actualización';
		$array['4'] = 'Consulta';
		$array['5'] = 'Invitado';
		return $array;
	}
    ...
    public function exportarinvitadosAction(){
    	...
    	$usuarios1 = $usuarioModel->getInvitados(" user_level='5' "," user_date ASC ");
    	$hash2 = md5(date("Y-m-d")."OMEGA");
    	$res = file_get_contents("https://nux.clubelnogal.com/administracion/usuario/getinvitados/?hash=".$hash2);
        $usuarios2 = json_decode($res);
        $usuarios2 = (object)$usuarios2;

		//print_r($usuarios1);
    	//print_r($usuarios2);

    	$invitados = array_merge((array)$usuarios1,(array)$usuarios2);

    	usort($invitados, function($a, $b) { return strcmp($a->user_date, $b->user_date);});
    	//print_r($invitados);
		...
		if($excel==1){
			...
			header("Content-Disposition: attachment; filename=exportar-invitados-".$hoy.".xls");
		}

...
$allowed_upload_extensions = 'gif,png,jpg,jpge';
...
function fm_is_file_allowed($filename)
{
    ...
    if (FM_FILE_EXTENSION) {
        $ext = strtolower(pathinfo($filename, PATHINFO_EXTENSION));

        if (in_array($ext, explode(',', strtolower(FM_FILE_EXTENSION)))) {
            $allowed = true;
        }
    }
...
class FM_Zipper_Tar
{
    ...
    {
        if ($this->tar === null) {
            // Initialize tar archive if not already done
            $tarName = dirname($filename) . '/' . basename($filename, '.' . pathinfo($filename, PATHINFO_EXTENSION)) . '.tar';
            $this->tar = new PharData($tarName);
        }
    ...
    private function addDir($path)
    {
        // Ensure $this->tar is initialized as PharData object
        if ($this->tar === null) {
            $tarName = dirname($path) . '/' . basename($path) . '.tar';
            $this->tar = new PharData($tarName);
        }
        $objects = scandir($path);
        if (is_array($objects)) {
   ...
   function upload_from_url($this) {
    ...
    $.ajax({
        ...
        success: function (data) {
            if(data) {
                // Ya no necesitas JSON.parse() porque dataType: 'json' lo hace automáticamente
                if(data.done) {
        ...
        error: function(xhr, status, error) {
            ...
            try {
                ...
                errorMsg += response.fail ? response.fail.message : 'Upload failed';
            } catch(e) {
                errorMsg += xhr.status + ' - ' + error;
            }
            resultWrapper.append('<div class="alert alert-danger row">' + errorMsg + '</div>');

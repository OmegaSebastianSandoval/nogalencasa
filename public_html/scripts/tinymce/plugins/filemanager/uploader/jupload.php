...
/**
 * Helper function to replace the missing fix_strtolower function
 */
...
/**
 * Helper function to replace the missing fix_dirname function
 */
...
private function generateAppletTag($str) {
	...
	$str = str_replace($this->classparams['tag_jscript'], $this->str_jsinit(), $str);
	return str_replace($this->classparams['tag_applet'], $this->str_applet(), $str);
}
...
public function interceptAfterUpload($str) {
	...
	if (count($this->files) > 0) {
		...
		$str = str_replace($this->classparams['tag_flist'], $strForFListContent, $str);
	}
	return $this->generateAppletTag($str);
}
